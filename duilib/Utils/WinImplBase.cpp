...
namespace DuiLib
{
    ...
    LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
    {
        ...
        if (GetResourceType() == UILIB_RESOURCE)
        {
            ...
            pRoot = builder.Create(GetSkinFile().GetData(), nullptr, this, &m_pm);
        }
        ASSERT(pRoot);
        if (pRoot == NULL)
        {
    ...
    CDuiString kWndBaseBackGround = _T("background"); //背景。图片 设置图片或颜色
    CDuiString kWndBaseMinBtn = _T("minbtn");
    CDuiString kWndBaseMinRestoreBtn = _T("minrestorebtn");
    CDuiString kWndBaseMaxBtn = _T("maxbtn");
    CDuiString kWndBaseMaxRestoreBtn = _T("maxrestorebtn");
    CDuiString kWndBaseCloseBtn = _T("closebtn");
