...
namespace DuiLib
{
    ...
    std::string CDuiString::GetStringA()
    {
        ...
        if (!IsEmpty()) {
			int iSize;
			char* pszMultiByte;
			iSize = WideCharToMultiByte(CP_ACP, 0, GetData(), -1, NULL, 0, NULL, NULL);
			pszMultiByte = (char*)malloc((iSize + 1)/**sizeof(char)*/);
			WideCharToMultiByte(CP_ACP, 0, GetData(), -1, pszMultiByte, iSize, NULL, NULL);
            string nRet = pszMultiByte;
            return nRet;
        }
        return "";
#else
